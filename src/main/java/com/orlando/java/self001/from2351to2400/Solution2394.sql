...
with cte as (
   select employee_id, sum(TIMESTAMPDIFF(HOUR, in_time, out_time)) as work_hour
   from logs
   group by employee_id
)
select e.employee_id
from Employees e left join cte on e.employee_id = cte.employee_id
where cte.employee_id is null or (cte.work_hour < e.needed_hours);
