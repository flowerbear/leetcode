...
select salesperson_id, name, sum(price)
from Salesperson s left join customer c on s.salesperson_id = c.salesperson_id left join Sales sl on c.customer_id = sl.customer_id
group by 1, 2;