...
select student_id, department_id, ROUND(ifnull(100 * (RANK() over (PARTITION by department_id order by mark desc) - 1) / (count(1) over (PARTITION by department_id) - 1), 0), 2) as percentage
from Students;