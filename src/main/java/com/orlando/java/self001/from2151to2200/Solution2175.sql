...
select team_id, name, cast(row_number() over (order by points desc, name asc) as SIGNED) - cast(row_number() over (order by points + points_change desc, name asc) as SIGNED) as rank_diff
from TeamPoints a inner join PointsChange b on a.team_id = b.team_id;